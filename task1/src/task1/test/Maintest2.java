...
import task1.implement.*;
import java.util.ArrayList;


//Test récuperer d'un camarade car je suis en retard.
...
public class Maintest2 {
    ...
    public static byte[] readSizeAndMessage(Channel channel) {
        ...
        while (bytesRead < messageSize) {
            int response = -1;
			try {
				response = channel.read(buffer, bytesRead, messageSize - bytesRead);
			} catch (DisconnectedException e) {
				e.printStackTrace();
			}
    ...
    public static void writeSizeAndMessage(Channel channel, byte[] message) {
        ...
        while (bytesWritten < buffer.length) {
            int response = -1;
			try {
				response = channel.write(buffer, bytesWritten, buffer.length - bytesWritten);
			} catch (DisconnectedException e) {
				e.printStackTrace();
			}
            if (response == -1) {
    ...
    public void test1() {
        Broker broker = new ImBroker("Broker1");

        Task serverTask = new ImTask(broker, new Runnable() {
            @Override
            public void run() {
                try {
        ...
        Task clientTask = new ImTask(broker, new Runnable() {
            @Override
            public void run() {
                try {
    ...
    public void test2() {
        Broker broker = new ImBroker("Broker1");
        Broker broker2 = new ImBroker("Broker2");
        ...
        Task clientTask = new ImTask(broker2, new Runnable() {
            @Override
            public void run() {
                try {
                    // Connect to the server on port 8080
    ...
    public void test3() {
        ...
        for (int i = 0; i < 1000; i++) {
            Broker broker = new ImBroker("Broker" + i);
            brokers.add(broker);
            Task serverTask = new ImTask(broker, new EchoServer(broker, true, "Broker" + i, i));
            Task clientTask = new ImTask(broker, new EchoServer(broker, false, "Broker" + i, i));
